...
@Test
public void testGetBlogById_ExistingBlog() {
    String blogId = "1";
    Blog existingBlog =  new Blog("1", "Title 1", "Author 1", List.of("Description 1"), List.of("tag1", "tag2"), "url1", "11:00PM", new ArrayList<>(), null);
    existingBlog.setId(blogId);
    when(blogPostRepository.findById(blogId)).thenReturn(Optional.of(existingBlog));
    ResponseEntity<Blog> responseEntity = blogController.getBlogById(blogId);
    assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
    assertEquals(existingBlog, responseEntity.getBody());
}

@Test
    public void testGetBlogById_NonExistingBlog() {
        String blogId = "123";
...
@Test
public void testAddBlog_Exception() {
    ...
    blogToAdd.setTitle("Test Blog");
    lenient().when(blogPostRepository.save(blogToAdd)).thenThrow(new RuntimeException("Some exception"));
    ResponseEntity<UserProfile> responseEntity = blogController.addBlog(blogToAdd, principal);
